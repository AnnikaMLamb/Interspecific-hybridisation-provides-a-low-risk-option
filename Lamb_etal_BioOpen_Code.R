...
###Early survivorship in captivity###
###Import data
survival<-read.csv('HybridFieldExperiment_PreDeployment_Survival.csv')
survival$TileID <-as.factor(survival$TileID )
survival$Cross<-as.factor(survival$Cross)
survival$ClusterPre <-as.factor(survival$ClusterPre)
survival$Tank <-as.factor(survival$Tank)
survival$Date <-as.Date(survival$Date, format = c("%d/%m/%Y"))
##Filter to singles
selected<-c("N")
survival_singles<-survival[survival$ClusterPre %in% selected,]
##Summarise survival
survival_singles_TileAverages<-summarySE(survival_singles, measurevar="Survivorship", groupvars=c("Cross","Tank","TileID"),conf.interval=0.95)
survival_singles_TileAverages
survival_singles_SE<-summarySE(survival_singles_TileAverages, measurevar="Survivorship", groupvars=c("Cross","Tank"),conf.interval=0.95)
survival_singles_SE
#Generalised linear mixed effects modelling
Survival_mod1<-glmer(Survivorship~Cross +(1|Tank/TileID) +(1|Date), data=survival_singles, family=binomial,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
emm_s.ct <- emmeans(Survival_mod1, pairwise ~ Cross ,adjust = "bonf")
emm_s.ct
